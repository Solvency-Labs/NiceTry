import Lake
open Lake DSL

package NiceTryForsVerity where
  version := v!"0.1.0"

require verity from git
  "https://github.com/lfglabs-dev/verity.git"@"bd211c574f45cda31d66feab1bbc7e9d08dc5486"

lean_lib NiceTry where
  srcDir := "."
  roots := #[
    `NiceTry.Fors.Types,
    `NiceTry.Fors.Hash,
    `NiceTry.Fors.Model,
    `NiceTry.Fors.TreeShape,
    `NiceTry.Fors.TreeKeccak,
    `NiceTry.Fors.FullKeccak,
    `NiceTry.Fors.RawKeccak,
    `NiceTry.Fors.Spec,
    `NiceTry.Fors.Proofs.Basic,
    `NiceTry.Fors.Proofs.TreeShape,
    `NiceTry.Fors.Proofs.TreeKeccak,
    `NiceTry.Fors.Proofs.FullKeccak,
    `NiceTry.Fors.Proofs.RawKeccak,
    `NiceTry.Fors.Verity.GuardKernel,
    `NiceTry.Fors.Verity.TreeShapeKernel,
    `NiceTry.Fors.Verity.TreeKeccakKernel,
    `NiceTry.Fors.Verity.FullVerifierKernel,
    `NiceTry.Fors.Bridge.RawDomain,
    `NiceTry.Fors.Bridge.Oracle,
    `NiceTry.Fors.Bridge.Equivalence,
    `NiceTry.Fors.Bridge.MemoryLayout,
    `NiceTry.Fors.Bridge.EvmFfiSpec,
    `NiceTry.Fors.Bridge.ByteArrayLemmas,
    `NiceTry.Fors.Bridge.EvmMemory,
    `NiceTry.Fors.Bridge.ForsRuntime,
    `NiceTry.Fors.Bridge.EvmRun,
    `NiceTry.Fors.Bridge.CalldataBytes,
    `NiceTry.Fors.Bridge.ClassA,
    `NiceTry.Fors.Bridge.ClassAConst,
    `NiceTry.Fors.Bridge.ClassARecover,
    `NiceTry.Fors.Bridge.ClassALen,
    `NiceTry.Fors.Bridge.FuelMono,
    `NiceTry.Fors.Bridge.AddressShape,
    `NiceTry.Fors.Bridge.Interp,
    `NiceTry.Fors.Bridge.InterpLoop,
    `NiceTry.Fors.Bridge.InterpOps,
    `NiceTry.Fors.Bridge.InterpState,
    `NiceTry.Fors.Bridge.InterpKeccak,
    `NiceTry.Fors.Bridge.InterpCall,
    `NiceTry.Fors.Bridge.InterpEval,
    `NiceTry.Fors.Bridge.InterpHash,
    `NiceTry.Fors.Bridge.TreeMemory,
    `NiceTry.Fors.Bridge.TreeLeaf,
    `NiceTry.Fors.Bridge.TreeNode,
    `NiceTry.Fors.Bridge.TreeValue,
    `NiceTry.Fors.Bridge.TreeIter,
    `NiceTry.Fors.Bridge.TreeArith,
    `NiceTry.Fors.Bridge.TreeLoop,
    `NiceTry.Fors.Bridge.TreeFinal,
    `NiceTry.Fors.Bridge.TreeCalldata,
    `NiceTry.Fors.Bridge.TreePreLoop,
    `NiceTry.Fors.Bridge.TreeEntry,
    `NiceTry.Fors.Bridge.TreeEntryFront,
    `NiceTry.Fors.Bridge.Refinement,
    `NiceTry.Fors.Bridge.EvmRunRecover,
    `NiceTry.Fors.Bridge.Phase4Accept,
    `NiceTry.Fors.Bridge.Phase4Reject,
    `NiceTry.Fors.Bridge.DispatcherRoute,
    `NiceTry.Fors.Bridge.Phase4,
    `NiceTry.Fors.Bridge.KernelRefinement
  ]
